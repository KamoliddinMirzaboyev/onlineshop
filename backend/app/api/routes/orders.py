...
def edit_my_order(
    ...
    """Mijoz do'kon hali tasdiqlamagan (pending) buyurtmasini tahrirlaydi:
    mahsulot qo'shish, miqdorni o'zgartirish, mahsulotni olib tashlash."""
    order = db.scalar(
        select(Order)
        .where(Order.id == order_id, Order.user_id == user.id)
    ...
    if not order:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Order not found")
    order = edit_pending_order(db, order, data.items)
    courier_events.publish({"type": "orders_updated", "restaurant_id": order.restaurant_id})
    return order
