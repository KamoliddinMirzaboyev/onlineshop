import { ChevronLeft, CircleCheck, CircleX, Minus, Plus, Search } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { toast } from "sonner";
import { get, post, withStore } from "../api";
...
/** Telefon orqali kelgan buyurtmani qo'lda qo'shish — admin panel bilan bir xil oqim. */
export default function ManualOrderModal({
    ...
    (async () => {
      try {
        const [cats, groups, products] = await Promise.all([
          get<Category[]>(withStore(`/admin/restaurants/${storeId}/categories`, storeId)),
          get<CategoryGroup[]>(withStore(`/admin/restaurants/${storeId}/category-groups`, storeId)),
          get<Product[]>(withStore(`/admin/restaurants/${storeId}/products`, storeId)),
        ]);
  ...
  const { cats, groups, products } = catalog ?? { cats: [], groups: [], products: [] };
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      await post(withStore("/admin/orders", storeId), {
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4">
          {loadErr ? (
            <div className="py-16 text-center text-slate-400">
              Katalog yuklanmadi.
              <button className="btn-ghost mt-3 mx-auto block" onClick={onClose}>Yopish</button>
            </div>
          ) : !catalog ? (
